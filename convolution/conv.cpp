...
int main(int argc, char *argv[])
{
  ...
  err = clEnqueueWriteBuffer(cv.commands, g_Output, true, 0, sizeof(float)*n*n,
          h_Output, 0, NULL, NULL);
  CHK_ERR(err);

  err = clEnqueueWriteBuffer(cv.commands, g_Input, true, 0, sizeof(float)*n*n,
          h_Input, 0, NULL, NULL);
  CHK_ERR(err);

  err = clEnqueueWriteBuffer(cv.commands, g_Weights, true, 0, sizeof(float)*n*n,
          h_Weights, 0, NULL, NULL);
  CHK_ERR(err);
  ...
  err = clSetKernelArg(conv, 3, sizeof(int), &n);
  CHK_ERR(err);

  err = clSetKernelArg(conv, 4, sizeof(int), &r);
  CHK_ERR(err);
  ...
  err = clEnqueueReadBuffer(cv.commands, g_Output, true, 0, sizeof(float)*n*n,
			    h_Output, 0, NULL, NULL);
  CHK_ERR(err);
  err = clFinish(cv.commands);
  CHK_ERR(err);
